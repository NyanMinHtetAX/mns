...
class ProductPricelist(models.Model):
    ...
    shelved_tags = fields.Boolean(string='Shelved Tags Label')
    is_field_sale = fields.Boolean(string="Is a fieldsales",copy=False)

class PricelistItemUom(models.Model):
    _inherit = 'pricelist.item.uom'

    shelved_tags = fields.Boolean(string='Shelved Tags Label', related='pricelist_id.shelved_tags', store=True)

class ProductTemplate(models.Model):
    _inherit = 'product.template'

    zawgyi_product_name = fields.Char(string='Product Name ', compute='get_zawgyi_name', store=True)
    pricelist_line_ids = fields.Many2many('product.pricelist.item', 'Price List UOM',
                                         compute='_compute_pricelist_line')
    shelve_name = fields.Char('Shelve Tab Name')
    zawgyi_shelve_name = fields.Char('Zawgyi Shelve Tab Name',  compute='get_zawgyi_shelve_name', store=True)

    def _compute_pricelist_line(self):
        pricelist_line = []
        for rec in self:
            if rec.id:
                pricelists_items = self.env['product.pricelist.item'].search([
                    ('product_id.product_tmpl_id', '=', rec.id),
                    ('pricelist_id.shelved_tags', '=', True),
                ])
                for item in pricelists_items:
                    pricelist_line.append(item.id)

            if pricelist_line:
                rec.pricelist_line_ids = pricelist_line
                pricelist_line = []
            else:
                rec.pricelist_line_ids = False
    ...
    @api.depends('shelve_name')
    def get_zawgyi_shelve_name(self):
        for rec in self:
            if rec.shelve_name:
                rec.zawgyi_shelve_name = rec.uni2zg(rec.shelve_name)
            else:
                rec.zawgyi_shelve_name = False
...
class PricelistUOM(models.Model):
    ...
    product_pricelist_id = fields.Many2one('product.template', related='product_id.product_tmpl_id')
