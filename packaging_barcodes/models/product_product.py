...
class ProductProduct(models.Model):
    ...
    related_barcodes = fields.Char(compute = '_compute_barcodes_from_packaging_ids', store=True)

    @api.depends('packaging_ids')
    def _compute_barcodes_from_packaging_ids(self):
        for product in self:
            product.related_barcodes = '|'.join([b for b in product.packaging_ids.mapped('barcode') if b])
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        if name:
            ...
            if operator in positive_operators:
                ...
                if not product_ids:
                    product_ids = list(self._search(['|',('barcode', '=', name),('related_barcodes','=',name)] + args, limit=limit, access_rights_uid=name_get_uid))
