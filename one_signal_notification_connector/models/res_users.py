...
class Users(models.Model):
    ...
    device_ids = fields.One2many(comodel_name="one.signal.notification.users.device",
                                 inverse_name="user_id", string="Device Ids", required=False, )


class OneSignalDeviceIDs(models.Model):
    _name = 'one.signal.notification.users.device'
    _rec_name = 'name'
    ...
    def update_one_signal_device(self, dictionary):
        ...
        :return: Json object
        a.	Input Request
            i.	denali_model = "one.signal.notification.users.device"
