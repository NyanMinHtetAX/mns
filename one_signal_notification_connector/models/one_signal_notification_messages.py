...
class OneSignalNotification(models.Model):
    ...
    def onchange_users(self):
        ...
        one_signal_user_object = self.env['one.signal.notification.users.device'].search(search_condition)
    ...
    def send_message(self):
        ...
        if self.app_id:
            ...
            apps_object = self.env['one.signal.notification.apps'].search([("id", "=", self.app_id.id),
                                                                            ("active", "=", True)],limit=1)
        else:
            # search_condition.append(("company_id", "=", self.env.user.company_id.id))
            apps_object = self.env['one.signal.notification.apps'].search([("company_id", "=", self.env.user.company_id.id),
                                                                            ("active", "=", True)],limit=1)

        # apps_object = self.env['one.signal.notification.apps'].search([search_condition],limit=1)
        # for app_record in apps_object.search(search_condition, limit=1):
        for app_record in apps_object:
