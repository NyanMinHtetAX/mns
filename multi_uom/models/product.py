...
class ProductTemplate(models.Model):
    ...
    def _compute_item_count(self):
        ...
        for template in self:
            template.pricelist_item_count = self.env['product.pricelist.item'].search_count(
...
class MultiUOMLine(models.Model):
    ...
    def _onchange_uom_id(self):
        for rec in self:
            if rec.product_tmpl_id:
                ...
                if rec.uom_id.id in product._origin.multi_uom_line_ids.mapped('uom_id').ids:
                    raise UserError('Choosed UOM is already used in another line.')
    ...
    def unlink(self):
        for record in self:
            ...
            moves = self.env['stock.move'].search([('product_id', 'in', product_ids), ('multi_uom_line_id', '=', record.id)])

            if moves:
                raise UserError(_('You can not delete this UOM Line'))
            else:
                # Also check that is also used in use Product's UOM and Purchase UOM 
                if len(record.product_tmpl_id.multi_uom_line_ids.filtered(lambda l: l.uom_id.id == record.uom_id.id)) == 1 and \
                    record.uom_id.id in [record.product_tmpl_id.uom_id.id, record.product_tmpl_id.uom_po_id.id]:
                    raise UserError(_('You can not delete this UOM Line'))                
